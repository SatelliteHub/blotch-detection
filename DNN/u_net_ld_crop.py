...
Original file is located at
    https://colab.research.google.com/drive/1B2HV6oLZKlixbpL8xirA1mzp4o4a1wqq
...
# Unzip the frames
frame_zip_path = "/content/drive/MyDrive/Colab Notebooks/LinesData/LinesData_Crop.zip"
frame_extract_dir = "/content"
with zipfile.ZipFile(frame_zip_path, 'r') as zip_ref:
    zip_ref.extractall(frame_extract_dir)

gt_frame_zip_path = "/content/drive/MyDrive/Colab Notebooks/LinesData/GT_LD_Crop.zip"
...
frames = []
startFrame = 1
endFrame = round(53 * 0.5)

firstCrop = 1
finalCrop = 37

for i in range(startFrame, endFrame):
    for j in range(firstCrop, finalCrop):
        frame_path = os.path.join("/content/LinesData_Crop/", f'LinesData{i:04d}_Crop{j:02d}.tiff')
...
for i in range(startFrame, endFrame):
  for j in range(firstCrop, finalCrop):
      gt_frame_path = os.path.join("/content/GT_LD_Crop", f'GT_LinesData_Binary{i:04d}_Crop{j:02d}.tiff')
      gt_frame = cv2.imread(gt_frame_path, cv2.IMREAD_GRAYSCALE)
      gt_frame = np.expand_dims(gt_frame, axis=-1)
      gt_frames.append(gt_frame)

print(frames[100].shape)
print(gt_frames[100].shape)
...
def unet_model(input_size=(240, 320, 1)):
...
for i in range(start_frame, end_frame):
    for j in range(first_crop, final_crop):
      # Loading
      frame_path = os.path.join("/content/croppedLD/", f'LinesData{i:04d}_Crop{j:02d}.tiff')
      frame = cv2.imread(frame_path, cv2.IMREAD_GRAYSCALE)
      ...
      prediction_path = os.path.join("/content/drive/MyDrive/Colab Notebooks/LinesData/LD_Predictions/",
                                    f'LD_PredictedMask{i:04d}_Crop{j:02d}.tiff')
