...
for i = startVer:endVer
    verDir{i} = sprintf('LD_PredictionsV%d', i);
end
...
for v = 1:length(verDir)
    for frame = startFrame:endFrame
        ...
        prFilepath_DNN = ['~/Documents/MAI Research/Videos/LinesData/' ...
            'DNN_LinesData/', verDir{v}, '/LD_PredictedMask%04d.tiff'];
        D_DNN = imread(sprintf(prFilepath_DNN, frame)) / 255;

        % Load ground truth frames
        gtFilePath = ['~/Documents/MAI Research/Videos/LinesData/' ...
            'GT_LinesData/GT_LinesData_1080/GT_LinesData_Binary%04d.tiff'];
        G = imread(sprintf(gtFilePath, frame));
